...
export const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-brand/90 backdrop-blur-md border-b border-line">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          {/* Logo */}
          <Link to="/" className="flex-shrink-0 flex items-center gap-2 cursor-pointer group">
            <div className="w-10 h-10 rounded-xl bg-gold flex items-center justify-center group-hover:bg-gold-hover transition-colors">
              <Building2 className="text-white w-6 h-6" />
            </div>
            <span className="text-2xl font-bold tracking-wider text-ink">达康控股</span>
          </Link>
                ...
                return (
                  <Link 
                    ...
                    className={`px-3 py-2 rounded-md text-sm font-medium transition-colors ${
                      isActive ? 'text-gold' : 'text-ink-muted hover:text-gold'
                    }`}
                  >
                    {item.name}
                  </Link>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div 
            ...
            className="md:hidden bg-brand-light border-b border-line overflow-hidden"
          >
                ...
                return (
                  <Link 
                    ...
                    className={`block px-3 py-2 rounded-md text-base font-medium transition-colors ${
                      isActive ? 'text-gold bg-sage' : 'text-ink-muted hover:text-gold hover:bg-sage'
                    }`}
                  >
                    {item.name}
                  </Link>
...
export const Footer = () => {
  return (
    <footer className="bg-sage pt-20 pb-10 border-t border-line mt-auto">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
          ...
          <div>
            <h4 className="text-ink font-medium mb-6">快速链接</h4>
